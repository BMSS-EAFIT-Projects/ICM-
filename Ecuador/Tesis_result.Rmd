...
df_all_methods <- readRDS("../data/prueba_falsasalarmas.rds")
...
df_CBF <- df_all_methods %>% filter(Method %in% c("KNN Constant BF + ICM", "MAD Constant BF + ICM", "IQR Constant BF + ICM"))
ggplot(df_CBF, aes(x = p_false_alarm, y = log_delay, color = Method)) +
  geom_line(linewidth = 0.6) +      
  geom_point(size = 1.5) +
  facet_grid(rows = vars(theta_stream), cols = vars(mu1),
             labeller = labeller(theta_stream = function(x) paste0("θ = ", x),
  ...
  labs(
    ...
    title = "Comparison: Constant Betting Function False alarm vs Mean Delay",
    subtitle = "Panels: θ = 100, 200 and μ₁ = 1, 1.5, 2",
    color = "Method"
  ) +
  theme(
...
```{r}
df_MBF <- df_all_methods %>% filter(Method %in% c("KNN Mixture BF + ICM", "MAD Mixture BF + ICM", "IQR Mixture BF + ICM"))
p2 <- ggplot(df_MBF, aes(x = p_false_alarm, y = log_delay, color = Method)) +
  geom_line(linewidth = 0.6) +      
  geom_point(size = 1.5) +
  facet_grid(rows = vars(theta_stream), cols = vars(mu1),
             labeller = labeller(theta_stream = function(x) paste0("θ = ", x),
  ...
  labs(
    x = expression(P[0](tau <= theta)),
    y = expression(log[10](1 + E[1](tau - theta ~ "|" ~ tau > theta))),
    title = "Comparison: Mixture Betting Function False alarm vs Mean Delay",
    subtitle = "Panels: θ = 100, 200 and μ₁ = 1, 1.5, 2",
    color = "Method"
  ) +
  theme(
...
```{r}
df_KDEF <- df_all_methods %>% filter(Method %in% c("KNN Precomputed KDE BF + ICM", "MAD Precomputed KDE BF + ICM", "IQR Precomputed KDE BF + ICM"))
p3 <- ggplot(df_KDEF, aes(x = p_false_alarm, y = log_delay, color = Method)) +
  geom_line(linewidth = 0.6) +      
  geom_point(size = 1.5) +
  facet_grid(rows = vars(theta_stream), cols = vars(mu1),
             labeller = labeller(theta_stream = function(x) paste0("θ = ", x),
  ...
  labs(
    x = expression(P[0](tau <= theta)),
    y = expression(log[10](1 + E[1](tau - theta ~ "|" ~ tau > theta))),
    title = "Comparison: Precomputed KDE Betting Function False alarm vs Mean Delay",
    subtitle = "Panels: θ = 100, 200 and μ₁ = 1, 1.5, 2",
    color = "Method"
  ) +
  theme(
...
```{r}
df_HIST <- df_all_methods %>% filter(Method %in% c("KNN Histogram BF + ICM", "MAD Histogram BF + ICM", "IQR Histogram BF + ICM"))
p4 <- ggplot(df_HIST, aes(x = p_false_alarm, y = log_delay, color = Method)) +
  geom_line(linewidth = 0.6) +      
  geom_point(size = 1.5) +
  facet_grid(rows = vars(theta_stream), cols = vars(mu1),
             labeller = labeller(theta_stream = function(x) paste0("θ = ", x),
  ...
  labs(
    x = expression(P[0](tau <= theta)),
    y = expression(log[10](1 + E[1](tau - theta ~ "|" ~ tau > theta))),
    title = "Comparison: Histogram Betting Function False alarm vs Mean Delay",
    subtitle = "Panels: θ = 100, 200 and μ₁ = 1, 1.5, 2",
    color = "Method"
  ) +
  theme(
...
```{r}
plot_delay_all_bf_noout <- function(threshold_col = "delay_10",
                               excluir_ncm  = "LR",
                               ncm_order    = c("KNN", "MAD", "IQR")) {

  df_plot <- table_results %>%
    dplyr::filter(!NCM %in% excluir_ncm) %>%
    dplyr::mutate(
      NCM          = factor(NCM, levels = ncm_order),
      mu1          = as.numeric(gsub(".*μ1=([0-9.]+)", "\\1", scenario_key)),
      theta_stream = as.numeric(gsub("θ=([0-9.]+)_.*", "\\1", scenario_key)),
      bf_label     = as.character(BF)
    ) %>%
    dplyr::rename(valor = !!threshold_col)

  lbl_threshold <- ifelse(threshold_col == "delay_5", "5%", "10%")

  ggplot(df_plot, aes(x = mu1, y = valor, color = NCM, group = NCM)) +
    geom_line(linewidth = 0.9) +
    geom_point(size = 1.6) +
    facet_grid(rows = vars(theta_stream), cols = vars(bf_label),
           labeller = labeller(
  theta_label = as_labeller(function(x) x, label_parsed),
  bf_label    = as_labeller(function(x) x)
)) +
    scale_x_continuous(breaks = c(1, 1.5, 2)) +
    labs(
      title = paste0("Probabilidad de falsa alarma ", lbl_threshold),
      x     = expression(mu[1]),
      y     = "Tiempo esperado de detección",
      color = "NCM"
    ) +
...
```{r}
p <- plot_delay_all_bf_noout("delay_10", excluir_ncm = "LR")
ggsave("delay_10_all_bf_noout.png", plot = p, width = 14, height = 7, dpi = 300)
```

```{r}
p <- plot_delay_all_bf_noout("delay_5", excluir_ncm = "LR")
ggsave("delay_5_all_bf_noout.png", plot = p, width = 14, height = 7, dpi = 300)
```
...
plot_por_bf <- function(df_mean, df_ref, bf_sel, ncol = 3, excluir_ncm = NULL) {
  dmean <- df_mean %>% filter(bf == bf_sel)
  dref  <- df_ref  %>% filter(bf == bf_sel)
  
  if (!is.null(excluir_ncm)) {
    dmean <- dmean %>% filter(!ncm %in% excluir_ncm)
  }
  
  # Convertir etiquetas a expresiones parseables
  # Asume formato "theta=100_mu1=1" → "theta==100~mu[1]==1"
  label_expr <- function(x) {
  x <- gsub("θ=([0-9.]+)_μ1=([0-9.]+)",
             "theta==\\1~mu[1]==\\2", x)
  return(x)
}
  
  dmean$scenario <- label_expr(as.character(dmean$scenario))
  dref$scenario  <- label_expr(as.character(dref$scenario))
...
plot_por_bf(summ_mean, ref_theta, bf_demo, ncol = 3, excluir_ncm='LR')
```
...
p <- plot_por_bf(summ_mean, ref_theta, bf_demo, ncol = 3,excluir_ncm='LR')
ggsave(
  filename = paste0("convergencia_tau_", bf_demo, ".png"),
  plot     = p,
  width    = 12,
  height   = 8,
  dpi      = 300
...
ggplot(sum_long,
  ...
  labs(x="Umbral (h)", y="Precision", color="NCM", linetype="Modo", shape="Modo") +
...
```{r}
contaminated_summary <- readRDS("../data/prueba_contaminada_scale.rds")
tau_contaminated <- readRDS("../data/prueba_taus_contaminada_scale.rds")
table_results_contaminated <-  readRDS("../data/tabla_delays_contaminada_scale.rds")
```
...
```{r}
df_CBF_contam <- contaminated_summary %>% filter(Method %in% c("KNN Constant BF + ICM (contaminado)", "MAD Constant BF + ICM (contaminado)", "IQR Constant BF + ICM (contaminado)"))
...
plot_delay_vs_mu1 <- function(bf_name,
                               ncm_order   = c("LR", "KNN", "MAD", "IQR"),
                               excluir_ncm = NULL) {
  ...
  if (!is.null(excluir_ncm)) x <- x %>% filter(!NCM %in% excluir_ncm)
  ncm_order <- ncm_order[ncm_order %in% unique(x$NCM)]

  df_plot <- x %>%
    dplyr::mutate(
      NCM           = factor(NCM, levels = ncm_order),
      theta_label   = paste0("theta==", theta_stream)   # parseable
    )

  ggplot(df_plot, aes(x = mu1, y = delay_5, color = NCM, group = NCM)) +
    geom_line(linewidth = 0.9) +
    geom_point(size = 1.6) +
    facet_wrap(vars(theta_label), scales = "free_y",
               labeller = label_parsed) +
    labs(
      title    = paste("Delay (FA 5%) vs μ₁ —", bf_name),
      subtitle = expression("Un panel por " ~ theta ~ ". Color por NCM."),
      x        = expression(mu[1]),
      y        = "Delay medio (umbral 5%)",
      color    = "NCM"
    ) +
    theme_minimal(base_size = 12) +
    theme(legend.position = "bottom")
...
```{r}
plot_delay_all_bf <- function(threshold_col = "delay_10",
                               excluir_ncm  = "LR",
                               ncm_order    = c("KNN", "MAD", "IQR")) {

  df_plot <- table_results_contaminated %>%
    dplyr::filter(!NCM %in% excluir_ncm) %>%
    dplyr::mutate(
      NCM          = factor(NCM, levels = ncm_order),
      mu1          = as.numeric(gsub(".*μ1=([0-9.]+)", "\\1", scenario_key)),
...
```{r}
p <- plot_delay_all_bf("delay_10", excluir_ncm = "LR")
ggsave("delay_10_all_bf_scale_outlier.png", plot = p, width = 14, height = 7, dpi = 300)
```
```{r}
plot_delay_all_bf("delay_5", excluir_ncm = "LR")
ggsave("delay_5_all_bf_scale_outlier.png", plot = p, width = 14, height = 7, dpi = 300)
```
...
p <- plot_por_bf(summ_mean_contam, ref_theta_contam, bf_demo, ncol = 3, excluir_ncm = 'LR')
ggsave(
  filename = paste0("convergencia_tau_contaminada_", bf_demo, ".png"),
  plot     = p,
  width    = 12,
  height   = 8,
  dpi      = 300
...
KNN_sens_summary <- KNN_sens_summary |> filter(!is.na(K))
```

##Mapa de calor retardeo medio para los diferentes valores de K

```{r}
pknn <- ggplot(KNN_sens_summary,
       aes(x = threshold, y = factor(K), fill = mean_delay)) +
  geom_tile(color = "white", size = 0.2) +
  facet_grid(bf ~ scenario_id, labeller = labeller(
    bf = label_value,              # cambia por nombres bonitos si quieres
    scenario_id = label_value
  )) +
  ...
  labs(
    x = "Threshold h",
    y = "K",
    title = "Sensibility of K – Mean Delay for h & K"
  ) +
...
# Asegura que 'ref' sea factor con los niveles esperados
vref <- vref |>
  dplyr::mutate(
...
vref <- vref %>%
  mutate(
    ref = recode(ref,
      "θ verdadero"        = "true θ",
      "τ estimado (MAD)"    = "estimated τ (MAD)",
      "τ estimado (PELT)"   = "estimated τ (PELT)",
      "τ estimado (IQR)"    = "estimated τ (IQR)",
      "τ estimado (KNN)"    = "estimated τ (KNN)"
    )
  )


p <- ggplot(sintetica_1, aes(t, y)) +
  ...
  scale_color_manual(
    ...
    values = c(
      "true θ"        = "#D55E00",
      "estimated τ (MAD)"   = "#0072B2",
      "estimated τ (PELT)"  = "#009E73",
      "estimated τ (IQR)"   = "#CC79A7",
      "estimated τ (KNN)"   = "red"
    ),
  ...
  scale_linetype_manual(
    ...
    values = c(
      "true θ"        = "solid",
      "estimated τ (MAD)"   = "solid",
      "estimated τ (PELT)"  = "dashed",
      "estimated τ (IQR)"   = "dashed",
      "estimated τ (KNN)"   = "dashed"
    ),
    breaks = levels(vref$ref),
    guide = "none"
  ) +
  guides(color = guide_legend(override.aes = list(linewidth = 1.5))) +
  labs(title = "Synthetic  time series with change point at 300",
...
ggsave("synthethic_onecgp.png", plot = p,
       width = 13.33, height = 7.5, units = "in", dpi = 150, bg = "white")
```



### Dos putnos de cambio en 500 y 1000 cambios de 0 a 1 y de 1 a 2
```{r}
...
p <- ggplot(multi_up, aes(t, y)) +
  ...
  guides(color = guide_legend(override.aes = list(linewidth = 1.5))) +
  labs(title = "Synthetic  time series with change points at 500 and 1000",
       x = "Time index", y = "Value") +
  coord_cartesian(expand = FALSE) +
  theme_minimal(base_size = 12) +
  theme(
...
ggsave("synthethic_twocgp.png", plot = p,
       width = 13.33, height = 7.5, units = "in", dpi = 150, bg = "white")
```

### Dos putnos de cambio en 500 y 1000 cambios de 1 a 0 y de 0 a 1
```{r}
...
vref_multiadapt <- vref_multiadapt %>%
  mutate(
    ref = recode(ref,
      ...
      "τ estimado (IQR)"    = "estimated τ (IQR)",
      "τ estimado (KNN)"    = "estimated τ (KNN)",
      "τ estimado (ICM multi)" = "estimated τ (ICM multi)"
    )
  )


p <- ggplot(multiadapt, aes(t, y)) +
  geom_line(linewidth = 0.3, color = "grey7") +
  geom_vline(
...
ggsave("synthethic_twocpupdn.png", plot = p,
       width = 13.33, height = 7.5, units = "in", dpi = 150, bg = "white")
```
...
ggplot(baseball_data, aes(year, y)) +
  ...
  guides(color = guide_legend(override.aes = list(linewidth = 1.5))) +
  labs(title = "Homerun rate differences",
       x = "Year", y = "Difference in HR rate") +
  coord_cartesian(expand = FALSE) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor = element_blank(),
...
#### RIO ANAMICHU
```{r}
obj_rioanamichu <- readRDS("../data/icm_regimes_rioanamichu.rds")

df        <- obj_rioanamichu$df
seg_mean  <- obj_rioanamichu$segments$mean
reg_stats <- obj_rioanamichu$stats

mean_glob <- obj_rioanamichu$globals$mean
...
ggplot(df, aes(t, y)) +
  ...
  geom_hline(
    yintercept = mean_glob,   # o: unique(reg_stats$mean_glob)
    linetype = "dotted", linewidth = 0.7, color = "black"
  ) +
  labs(
    title = obj_rioanamichu$meta$title_mean,
    x = "Fecha",
    y = obj_rioanamichu$meta$y_label
  ) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank())
```
...
#### RIO RIONEGRO
```{r}
obj_riorionegro <- readRDS("../data/icm_regimes_riorionegro.rds")

df        <- obj_riorionegro$df
seg_mean  <- obj_riorionegro$segments$mean
reg_stats <- obj_riorionegro$stats

mean_glob <- obj_riorionegro$globals$mean
...
ggplot(df, aes(t, y)) +
  ...
  labs(
    title = obj_riorionegro$meta$title_mean,
    x = "Fecha",
    y = obj_riorionegro$meta$y_label
  ) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank())
```
