...
df_CBF <- df_all_methods %>% filter(Method %in% c("KNN Constant BF + ICM", "LR Constant BF + ICM", "MAD Constant BF + ICM", "Posterior Oracle", "CUSUM Oracle", "S-R Oracle"))
...
este mapa de calor evidencia como para diferentes valores k el promedio de los retardos medios varia en algunos casos mas evidente que en otros como por ejemoplo la funcion de apuesta histograma la cual evidencia mayor variabilidad cuando el cambio es pequeño es decirf salto en la media mu=1, para los demas escenarios el comportamiento parece mas uniforme salvo en los valores de K= 1 y k=200 donde parece haber un incremento en los retardos medios.

Para exhibir una mejor comparativa se presentan las siguientes graficas 
