...
df_fig_3_MAD_mixt <- data.frame(time = 1:n)
...
res_cp <- ICM(training_set, stream_data, Non_conformity_MAD, Mixture_BF)

df_fig_3_MAD_mixt$con_cp <- res_cp$Cn
...
for (i in 1:5) {
  ...
  res_m <- ICM(training_set_wcp, stream_data_wcp, Non_conformity_MAD, Mixture_BF)
  
  df_fig_3_MAD_mixt[[paste0("m_", i)]] <- res_m$Cn
}

# Guardar para graficar en el notebook
saveRDS(df_fig_3_MAD_mixt, file = "data/figura3_icm_MAD_mixt.rds")