...
Series <- read_excel("C:/Users/USER/Desktop/ICM_Ms_Degree/Archivos/Series.xlsx")
```
...
# -------------------------------------------------------------------
# 2) Precomputed KDE Betting Function
# -------------------------------------------------------------------
Precomputed_KDE_Betting_Func <- function(training_set,
                                         calibration_data,
...
##ICM
```{r}
ICM <- function(training_set, data, ncm, betting_function, k = 1, th = 1) {
  # Barajar el conjunto de entrenamiento
  training_set <- sample(training_set)
  
  n            <- length(data)
  valores_alfa <- numeric(n)
  p_valores    <- numeric(n)
  g_valores    <- numeric(n)
  Sn           <- numeric(n)
  Cn           <- numeric(n)
  ...
  for (i in seq_len(n)) {
    ...
    # 3) función de apuesta gᵢ
    g_valores[i] <- betting_function(p_valores[i])
    
    # 4) actualizar martingala Sᵢ = S_{i-1} · gᵢ
    Sn[i]   <- S_prev * g_valores[i]
    S_prev  <- Sn[i]
    
    # 5) conformal Cᵢ = log Sᵢ − log (min Sⱼ for j≤i)
    min_S    <- min(min_S, Sn[i])
    Cn[i]    <- log(S_prev) - log(min_S)
  }
  
  # Localizar el primer punto donde Cn > th
...
## Wrappers para facilidad en la implementacion
```{r}
ICM_knn_kde <- function(training_set,
                                    calibration_data,
                                    data,
                                    k = 1,
                                    n_grid = 512,
                                    th = 1) {
  # 1) Construir la betting function estática
  kde_bf <- Precomputed_KDE_Betting_Func(
  ...
  # 2) Llamar a tu ICM genérico
  ICM(
    training_set     = training_set,
    data             = data,
    ncm              = Non_conformity_KNN,
    betting_function = kde_bf,
    k                = k,
    th               = th
  )
}
...
abline(v = 300 - training_size, col = "red", lty = 2, lwd = 2)
...
```{r}
set.seed(123)
resultado <- ICM(entrenamiento, serie_de_tiempo, ncm = Non_conformity_KNN, betting_function = Mixture_Betting_Func, k = ceiling(100/2), th=2.8)
#resultado
```

```{r}
plot(resultado$Cn, type = 'l')
```
...
```{r}
set.seed(123)
resultado_KDE <-ICM_knn_kde(entrenamiento, calibration_data = serie_de_tiempo[1:50], serie_de_tiempo[-(1:50)],k = ceiling(100/2),th = 3.2)
```

```{r}
plot(resultado_KDE$Cn, type = 'l')
abline(v=50)
```


## Grafica Usando Mixture betting function
```{r}
datos_mixt <- data.frame(
  tiempo = 1:length(datos),
  cn = datos
)

grafica_sinteticos<-ggplot(datos_mixt, aes(x = tiempo, y = cn)) +
  geom_line(color = "black") +
  # Agregar líneas verticales con color según tipo
  geom_vline(aes(xintercept = resultado$change_point + training_size, color = "ICM"), linetype = "dashed", linewidth = 1) +
...
grafica_sinteticos
```
## Grafica usando KDE Betting Function
```{r}
datos_kde <- data.frame(
  tiempo = 1:length(datos),
  cn = datos
)

grafica_sinteticos_KDE<-ggplot(datos_kde, aes(x = tiempo, y = cn)) +
  geom_line(color = "black") +
  # Agregar líneas verticales con color según tipo
  geom_vline(aes(xintercept = resultado_KDE$change_point + 150, color = "ICM"), linetype = "dashed", linewidth = 1) +
  geom_vline(aes(xintercept = theta, color = "Punto de conocido"), linetype = "dashed", size = 1) +
  scale_color_manual(
    name = "Líneas",
    values = c("ICM" = "red", "Punto de conocido" = "blue")
...
```{r}
resultado_KDE$change_point
```

```{r}
grafica_sinteticos_KDE
```
...
for (m in c(1,2,3,4,5)) {
  ...
  resultado <- ICM(entrenamiento, serie_de_tiempo, ncm = Non_conformity_KNN, betting_function = Mixture_Betting_Func, k = ceiling(m/2))
...
Cn <- ICM(entrenamiento_achicaya$valor, serie_de_tiempo_anchicaya$valor, ncm = Non_conformity_KNN, betting_function = Mixture_Betting_Func, k = ceiling(75/2))
```
...
resultado <- ICM(entrenamiento_baseball, serie_de_tiempo_baseball, ncm = Non_conformity_KNN, betting_function = Mixture_Betting_Func, k = ceiling(10/2), th=3)
resultado
```
```{r}
tible <- mlb_diffs
